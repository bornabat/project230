#' Optimize the Rosenbrock's function using the Fletcher-Reeves algorithm
#'
#' Optimize the Rosenbrock's function using the Fletcher-Reeves algorithm.
#'
...
#'
#' @examples
#' rosenopt_fr(n.iter=1e5)
#'
#' @export
rosenopt_fr = function(init = c(5,5), n.iter=1e3, alpha=.001) {
  alp.org=alpha
  ...
  for (i in 1:n.iter) {
    ...
    if (lk.candid < lk) {
      beta = sum(rosen_grad(vals.candid[1], vals.candid[2])^2) / sum(grads^2)  # compute beta using Fletcher-Reeves formula
      d = -rosen_grad(vals.candid[1], vals.candid[2]) + beta * d  # update direction using Fletcher-Reeves formula
      vals = vals.candid
      lk = lk.candid
      grads = rosen_grad(vals[1], vals[2])
