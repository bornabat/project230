#' Optimize the Rosenbrock's function using the RMSprop algorithm
#'
#' Optimize the Rosenbrock's function using the RMSprop algorithm.
#'
...
#'
#' @examples
#' rosenopt_rmsprop(init = c(5, 5), n.iter = 1e3, alpha = 1, epsilon = 1e-6)
#' @export
rosenopt_rmsprop = function(init = c(5,5), n.iter=1e3, alpha=0.1, epsilon=1e-6, gamma=0.9) {
  alp.org = alpha
  ...
  for (i in 1:n.iter) {
    g = gamma * g + (1 - gamma) * grads^2  # update RMSprop moving average
    vals.candid = vals - alpha * grads / (sqrt(g) + epsilon)  # update candidate position using RMSprop formula
