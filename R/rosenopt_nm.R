#' Optimize the Rosenbrock's function using the nesterov momentum algorithm
#'
#' Optimize the Rosenbrock's function using the nesterov momentum algorithm.
#'
...
#'
#' @examples
#' rosenopt_nm(n.iter=1e5)
#'
#' @export
rosenopt_nm = function(init = c(5,5), n.iter=1e3, alpha=1, gamma=0.9) {
  alp.org = alpha
  gam.org = gamma
  ...
  for (i in 1:n.iter) {
    vals_lookahead = vals + gamma * v
    grads_lookahead = rosen_grad(vals_lookahead[1], vals_lookahead[2])
    v.candid = gamma * v - alpha * grads_lookahead
    vals_candid = vals + v.candid
    lk.candid = rosenbrock(vals_candid[1], vals_candid[2])
